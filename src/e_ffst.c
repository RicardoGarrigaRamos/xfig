...
sel_line(int xmin, int ymin, int xmax, int ymax) { 
 ...
 {  
	 l->fill_color = cur_fillcolor;
       l->fill_style = cur_fillstyle;
...
init_arb_move(F_line* p, int type, int x, int y, int px, int py) { 
 ...
 p->fill_color = cur_fillcolor;
 p->fill_style = cur_fillstyle;
 canvas_middlebut_proc = null_proc; 
...
init_move(F_line* p, int type, int x, int y, int px, int py) { 
 ...
 switch (type) { 
	 ...
	 case O_POLYLINE: 
		 ...
		 cur_l->fill_color = cur_fillcolor;
      	 cur_l->fill_style = cur_fillstyle;
		 break; 
	 case O_TXT: 
		 set_cursor(wait_cursor); 
		 cur_t = (F_text*)p; 
